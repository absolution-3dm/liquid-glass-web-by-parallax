...
function ColorPickerHueSlider(
  ...
  return (
    ...
    >
      ...
      <SliderPrimitive.Thumb className="block size-4 rounded-full border border-oklch(0.922 0 0) border-oklch(0.205 0 0)/50 bg-oklch(1 0 0) shadow transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-oklch(0.708 0 0) disabled:pointer-events-none disabled:opacity-50 dark:border-oklch(1 0 0 / 10%) dark:border-oklch(0.922 0 0)/50 dark:bg-oklch(0.145 0 0) dark:focus-visible:ring-oklch(0.556 0 0)" />
    </SliderPrimitive.Root>
  );
}
  ...
  return (
    ...
    >
      <SliderPrimitive.Track
        ...
        style={{
          background:
            "linear-gradient(45deg, #ccc 25%, transparent 25%), linear-gradient(-45deg, 75%, 75%), 75%)",
          backgroundSize: "8px",
          backgroundPosition: "0 0, 4px, 4px -4px, -4px 0px",
        }}
      >
        <div
...
function ColorPickerSwatch(props: DivProps) {
  ...
  const backgroundStyle = React.useMemo(() => {
    if (!color) {
      return {
        background:
          "linear-gradient(to bottom right, transparent calc(50% - 1px), hsl(var(--destructive)) 1px) + 1px)) no-repeat",
      };
    }
  ...
  return (
    <SwatchPrimitive
      ...
      {...swatchProps}
      className={cn(
        "box-border size-8 rounded-sm border border-oklch(0.922 0 0) shadow-sm dark:border-oklch(1 0 0 / 10%)",
